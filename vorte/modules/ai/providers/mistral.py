"""
Vorte AI Module - Mistral Provider
====================================
Provider implementation for the Mistral AI API (La Plateforme).
"""
...
logger = logging.getLogger("vorte.ai.providers.mistral")
...
class MistralProvider(BaseProvider):
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    logger.warning("Mistral HTTP %s (attempt %d/%d): %s",
                                   exc.response.status_code, attempt, self.max_retries, body[:300])
                    if exc.response.status_code in (401, 403):
                        raise RuntimeError(f"Mistral auth error: {body[:200]}") from exc
                    ...
                    if attempt < self.max_retries:
                        ...
                        continue
                    raise
            raise RuntimeError(f"Mistral request failed after {self.max_retries} retries") from last_exc
    ...
    def _translate_messages(self, request: CompletionRequest) -> List[Dict[str, Any]]:
        ...
        # Vision: add image(s) to last user message
        images: List[str] = []
        if request.image:
    ...
    async def complete(self, request: CompletionRequest) -> AIResponse:
        ...
        return AIResponse(
            ...
            provider=self.name,
            finish_reason=_MISTRAL_FINISH_MAP.get(
            ...
            async with client.stream(
                ...
                async for line in resp.aiter_lines():
                    ...
                    yield StreamChunk(
                        ...
                        provider=self.name,
                        finish_reason=_MISTRAL_FINISH_MAP.get(choice.get("finish_reason")),
                        delta=delta if delta else None,
                        request_id=request.request_id,
                    )

    async def embed(self, request: EmbeddingRequest) -> EmbeddingResponse:
        model = self.model_id(request.model) or "mistral-embed"
